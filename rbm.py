...
# Achari Berrada Youssef comment : 
# This is the implementation of the RBM 
# It was available in the Deep_Learning_Tensorflow Repository with 'bin' and 'gauss' 
# I added the 'rsm' in order to support the Replicated Softmax Model needed for the multi_DBM model
...
class RBM(UnsupervisedModel):
    ...
    def sample_visible_from_hidden(self, hidden, n_features):
        ...
        if self.visible_unit_type == 'bin':
            ...
            vstates = tf.multinomial(vprobs, 1)
            #vstates = []
            #with tf.Session() as sess:
            #    for i, vp in enumerate(sess.run(vprobs)):
            #        vstates.append(
